...
class FlowList(Flow):
    ...
    def forward(self, x, y=None, compute_jacobian=True):
        ...
        for flow in self.flow_list:
            ...
            if compute_jacobian:
                # if  type(flow).__name__  not in ["Squeeze", "Unsqueeze", "Preprocess"]:
                #     print('mean', flow.logdet_jacobian.mean())
                logdet_jacobian = logdet_jacobian + flow.logdet_jacobian

        if compute_jacobian:
            self._logdet_jacobian = logdet_jacobian
            # print('update log_det_jacobian {}'.format(self._logdet_jacobian.mean()))
        return x
